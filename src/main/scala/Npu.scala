...
object Npu extends App {
    ...
    SpinalConfig(headerWithDate = true, oneFilePerComponent = false, defaultConfigForClockDomains = clockCfg, targetDirectory = Config.filePath + File.separator + "rtl").generateVerilog(new Npu(ConvConfig(8, 16, 16, 12, 4096, 512, 640, 4096, 1), ShapeConfig(8, 8, 640, 12, 4096)))
    TotalTcl(Config.filePath + File.separator + "tcl", Config.filePath).genTotalTcl
}
