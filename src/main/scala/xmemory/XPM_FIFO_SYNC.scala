...
class XPM_FIFO_SYNC(config: XPM_FIFO_SYNC_CONFIG) extends BlackBox{
    ...
    val io = new Bundle {
        ...
        val dout = out Bits (config.READ_DATA_WIDTH bits)
        val empty = out Bool()
        val full = out Bool()
        val overflow = out Bool()
        val prog_empty = out Bool()
        val prog_full = out Bool()
        val rd_data_count = out Bits (config.RD_DATA_COUNT_WIDTH bits)
        val rd_rst_busy = out Bool()
        val sbiterr = out Bool()
        val underflow = out Bool()
        val wr_ack = out Bool()
        val wr_data_count = out Bits (config.WR_DATA_COUNT_WIDTH bits)
        val wr_rst_busy = out Bool()
        val din = in Bits (config.WRITE_DATA_WIDTH bits)
