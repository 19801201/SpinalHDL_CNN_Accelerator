...
class InstructionGen(simConfig: ConvSimConfig, convConfig: ConvConfig) extends Component {
    ...
    val in = Bits(32 * 4 bits) simPublic()
    in := (CONV_STATE.ROW_NUM_IN -> B(simConfig.rowNumIn), CONV_STATE.COL_NUM_IN -> B(simConfig.colNumIn), CONV_STATE.CHANNEL_IN -> B(simConfig.channelIn),
        ...
        CONV_STATE.EN_STRIDE -> simConfig.enStride, CONV_STATE.FIRST_LAYER -> simConfig.firstLayer, CONV_STATE.QUAN_NUM -> B(quanNum), CONV_STATE.WEIGHT_NUM -> B(weightNum), default -> false)
    val instruction = in.subdivideIn(4 slices) simPublic()
...
//    val simConfig = ConvSimConfig(416, 416, 32, 64, true, true, 68, 1, 68, CONV_STATE.CONV33, 2400, 692224, false, false, 1114133,"simData/all_weight_new.mem", "simData/test_416.mem")
    val simConfig = ConvSimConfig(160, 160, 128, 128, true, true, 14, 1, 67, CONV_STATE.CONV11, 2240, 409600, false, false, 1114133,"simData/conv11/weight_rs1_conv4.mem", "simData/conv11/feature_real825_640_rs1_conv4_leak.mem")
